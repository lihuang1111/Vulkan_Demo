...
#define CALL_VK(func)                                                 \
  if (VK_SUCCESS != (func)) {                                         \
    printf( "Tutorial Vulkan error. File[%s], line[%d]", __FILE__,    \
                        __LINE__);                                    \
    assert(false);                                                    \
  }
...
void CreateSwapChain(void) {
  ...
  delete[] formats;
  printf("<-createSwapChain end\n");
}

void DeleteSwapChain(void) {
  for (int i = 0; i < (int8_t)swapchain.swapchainLength_; i++) {
...
bool InitVulkan(ANativeWindow* platformWindow) {
  //androidAppCtx = app;

  if (!InitVulkan()) {
    printf("Vulkan is unavailable, install vulkan and re-start");
    return false;
  }
  ...
  // create a device
  CreateVulkanDevice(platformWindow, &appInfo);
  ...
  for (int bufferIndex = 0; bufferIndex < (int)swapchain.swapchainLength_;
       bufferIndex++) {
